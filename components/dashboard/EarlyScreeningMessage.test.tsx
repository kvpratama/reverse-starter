  ...
  ParticipateModal: ({ open, onClose, onSuccess, jobPost, profileId }: any) => 
    open ? (
      <div data-testid="participate-modal">
        <button onClick={() => { onSuccess(); onClose(); }} data-testid="participate-submit">
          Submit Participation
        </button>
        <button onClick={onClose} data-testid="participate-close">Close</button>
      </div>
    ) : null,
}));
...
describe('EarlyScreeningMessage', () => {
  const mockMessage: Message = {
    id: '1',
    content: 'This is a test early screening message with <strong>HTML content</strong>',
    jobPostId: 'job-123',
    timestamp: new Date(),
  ...
  const mockJobPost: JobPost = {
    ...
    questions: [
      ...
      { id: 2, question: 'Are you available to start immediately?', type: 'boolean' }
    ]
  };
  ...
  describe('Component Rendering', () => {
    it('renders message content with HTML', async () => {
      ...
      const messageContent = screen.getByText((content, element) => 
        element?.innerHTML === 'This is a test early screening message with <strong>HTML content</strong>'
      );
      expect(messageContent).toBeInTheDocument();
    });

    it('renders all action buttons', async () => {
      await renderAsync(<EarlyScreeningMessage msg={mockMessage} profileId="profile-123" />);
      ...
      await waitFor(() => {
        ...
        expect(screen.getByTestId('button-view-your-profile')).toBeInTheDocument();
      });
    });
  ...
  describe('Modal Interactions', () => {
    ...
    describe('Job Post Modal', () => {
      it('opens job post modal when Check Job Post button is clicked', async () => {
        ...
        await waitFor(() => {
          expect(screen.getByTestId('job-post-details-card')).toBeInTheDocument();
        });
      });
      ...
      it('closes job post modal when backdrop is clicked', async () => {
        ...
        await waitFor(() => {
          expect(screen.queryByTestId('job-post-details-card')).not.toBeInTheDocument();
        });
      });
    });
    ...
    describe('Profile Modal', () => {
      it('disables profile button when no profileId is available', async () => {
        await renderAsync(<EarlyScreeningMessage msg={mockMessage} profileId={undefined} />);
  ...
  describe('Edge Cases and Error Handling', () => {
    ...
    it('handles component unmounting during async operations', async () => {
      ...
      const { unmount } = await renderAsync(<EarlyScreeningMessage msg={mockMessage} />);
  ...
  describe('Accessibility', () => {
    ...
    it('has proper button labeling', async () => {
      ...
      expect(screen.getByTestId('button-check-job-post')).toHaveTextContent('Check Job Post');
      expect(screen.getByTestId('button-participate')).toHaveTextContent('Participate');
      expect(screen.getByTestId('button-view-your-profile')).toHaveTextContent('View Your Profile');
    });
  ...
  describe('Performance Considerations', () => {
    ...
    it('does not make unnecessary API calls on re-renders', async () => {
      const { rerender } = await renderAsync(<EarlyScreeningMessage msg={mockMessage} />);
