...
jest.mock('@/components/dashboard/JobseekerProfileCardUI', () => ({
  JobseekerProfileCardUI: ({ profile, screeningQuestions, screeningAnswers }: any) => (
...
const mockCandidate: Candidate = {
  ...
  reasoning: 'This candidate has excellent skills matching the job requirements with 5 years of relevant experience.',
...
describe('CandidatesCard', () => {
  ...
  it('displays progress rings with correct scores', () => {
    ...
    expect(screen.getByTestId('progress-ring-overall-match')).toHaveTextContent('Overall Match: 85%');
    expect(screen.getByTestId('progress-ring-bio-match')).toHaveTextContent('Bio Match: 75%');
    expect(screen.getByTestId('progress-ring-skills-match')).toHaveTextContent('Skills Match: 90%');
  });
  ...
  it('displays screening questions and answers in profile modal', async () => {
    ...
    expect(screen.getByText('Q: How many years of React experience do you have?')).toBeInTheDocument();
    expect(screen.getByTestId('screening-answers')).toBeInTheDocument();
    expect(screen.getByText('A: Yes, I have 5 years of experience with React')).toBeInTheDocument();
  });
  ...
  it('opens invite modal when "Invite For Interview" is clicked', async () => {
    ...
    expect(screen.getByPlaceholderText('https://calendly.com/your-link')).toBeInTheDocument();
  });
  ...
  it('validates URL input in invite modal', async () => {
    ...
    const urlInput = screen.getByPlaceholderText('https://calendly.com/your-link');
    ...
    fireEvent.change(urlInput, { target: { value: 'https://calendly.com/test-link' } });
  ...
  it('displays AI reasoning in profile modal', async () => {
    ...
    expect(screen.getByText('AI can make mistakes — please verify any critical information for accuracy')).toBeInTheDocument();
