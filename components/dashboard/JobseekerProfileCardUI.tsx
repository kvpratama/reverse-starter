      ...
      <CardContent className="p-6 space-y-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          ...
          <div className="space-y-2">
            ...
            <p className="font-semibold text-gray-900">{profile.age ?? "Not specified"}</p>
          </div>
          ...
          <div className="space-y-2">
            ...
            <p className="font-semibold text-gray-900">{profile.visaStatus || "Not specified"}</p>
          </div>
          ...
          <div className="space-y-2">
            ...
            <p className="font-semibold text-gray-900">{profile.nationality || "Not specified"}</p>
          </div>
        </div>
        ...
        <div className="space-y-3">
          ...
          <div className="flex flex-wrap items-center gap-2">
            {profile.jobCategory?.name && (
              <Badge variant="secondary" className="bg-orange-100 text-orange-800 hover:bg-orange-200 px-3 py-1">
                {profile.jobCategory.name}
              </Badge>
            )}
            {profile.jobSubcategory?.name && (
              <>
                ...
                <Badge variant="secondary" className="bg-orange-200 text-orange-900 hover:bg-orange-300 px-3 py-1">
                  {profile.jobSubcategory.name}
                </Badge>
              </>
            )}
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-4">
              {profile.workExperience.map((we, index) => (
                <div key={we.id} className="bg-white border border-gray-200 rounded-lg p-4 shadow-sm hover:shadow-md transition-shadow">
                  <div className="flex items-start justify-between mb-2">
                    <div>
                      <h4 className="font-semibold text-gray-900">{we.position || "Position not specified"}</h4>
                      <p className="text-orange-600 font-medium">{we.company || "Company not specified"}</p>
                    </div>
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-4">
              {profile.education.map((ed, index) => (
                <div key={ed.id} className="bg-white border border-gray-200 rounded-lg p-4 shadow-sm hover:shadow-md transition-shadow">
                  <div className="flex items-start justify-between mb-2">
                    <div>
                      <h4 className="font-semibold text-gray-900">{ed.degree || "Degree not specified"}</h4>
                      <p className="text-orange-600 font-medium">{ed.institution || "Institution not specified"}</p>
                      {ed.fieldOfStudy && (
                        <p className="text-sm text-gray-600 mt-1">Field: {ed.fieldOfStudy}</p>
                      )}
                    </div>
