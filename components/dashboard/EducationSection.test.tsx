...
jest.mock("lucide-react", () => ({
  GraduationCap: ({ className }: any) => (
    <div className={`icon graduation-cap ${className}`} />
  ),
  Calendar: ({ className }: any) => (
    <div className={`icon calendar ${className}`} />
  ),
  BookOpen: ({ className }: any) => (
    <div className={`icon book-open ${className}`} />
  ),
  School: ({ className }: any) => (
    <div className={`icon school ${className}`} />
  ),
  FileText: ({ className }: any) => (
    <div className={`icon file-text ${className}`} />
  ),
}));
...
const mockEducation: Education = {
  ...
  description:
    "Graduated magna cum laude with a focus on artificial intelligence and machine learning. Completed senior thesis on neural networks.",
};
...
const mockEducationArray: Education[] = [
  mockEducation,
  {
    ...
    institution: "MIT",
    description:
      "Currently pursuing advanced studies in statistical modeling and big data analytics.",
  },
];
...
describe("EducationSection", () => {
  describe("Rendering", () => {
    ...
    it("renders nothing when educations is not an array", () => {
      const { container } = render(
        <EducationSection educations={null as any} />,
      );
      expect(container.firstChild).toBeNull();
    });
    ...
    it("renders education cards when educations array has items", () => {
      ...
      expect(
        screen.getByDisplayValue("Stanford University"),
      ).toBeInTheDocument();
  ...
  describe("Form Fields", () => {
    it("renders all form fields with correct names and IDs", () => {
      ...
      expect(screen.getByRole("textbox", { name: /degree/i })).toHaveAttribute(
        "name",
        "education[0][degree]",
      );
      expect(
        screen.getByRole("textbox", { name: /field of study/i }),
      ).toHaveAttribute("name", "education[0][field_of_study]");
      expect(
        screen.getByRole("textbox", { name: /institution/i }),
      ).toHaveAttribute("name", "education[0][institution]");
      expect(
        screen.getByRole("textbox", { name: /start date/i }),
      ).toHaveAttribute("name", "education[0][start_date]");
      expect(
        screen.getByRole("textbox", { name: /end date/i }),
      ).toHaveAttribute("name", "education[0][end_date]");
      expect(
        screen.getByRole("textbox", { name: /description/i }),
      ).toHaveAttribute("name", "education[0][description]");
    });
    ...
    it("uses custom namePrefix when provided", () => {
      render(
        <EducationSection
          ...
          namePrefix="custom_education"
        />,
      );

      expect(screen.getByRole("textbox", { name: /degree/i })).toHaveAttribute(
        "name",
        "custom_education[0][degree]",
      );
      expect(
        screen.getByRole("textbox", { name: /field of study/i }),
      ).toHaveAttribute("name", "custom_education[0][field_of_study]");
    });
    ...
    it("shows correct placeholders for empty fields", () => {
      ...
      expect(
        screen.getByPlaceholderText("e.g. Bachelor of Science, Master of Arts"),
      ).toBeInTheDocument();
      expect(
      ...
      expect(
        screen.getByPlaceholderText("e.g. Stanford University, MIT"),
      ).toBeInTheDocument();
      expect(
        screen.getByPlaceholderText("e.g. September 2018"),
      ).toBeInTheDocument();
      expect(
        screen.getByPlaceholderText("e.g. May 2022 or Present"),
      ).toBeInTheDocument();
      expect(
        screen.getByPlaceholderText(
          "Describe relevant coursework, academic achievements, honors, GPA, thesis work, or extracurricular activities...",
        ),
      ).toBeInTheDocument();
    });
  });
  ...
  describe("Disabled State", () => {
    ...
    it("enables all form fields when disabled prop is false", () => {
      render(
        <EducationSection educations={[mockEducation]} disabled={false} />,
      );
  ...
  describe("Multiple Education Entries", () => {
    it("renders multiple education entries with correct indexing", () => {
      ...
      expect(screen.getByDisplayValue("Bachelor of Science")).toHaveAttribute(
        "name",
        "education[0][degree]",
      );
      expect(screen.getByDisplayValue("Master of Science")).toHaveAttribute(
        "name",
        "education[1][degree]",
      );
    });
